...
AmrCore::regrid (int lbase, Real time, bool)
{
    ...
    // DEBUG
    // Print() << std::format("***** AmrCore::regrid({:d})\n", lbase);
    // Print() << std::format("***** new_finest = {:d}, finest_level = {:d}\n", new_finest, finest_level);
    // Print() << std::format("***** new_grids.size() = {:d}\n", new_grids.size());
    // for (int lev = 0; lev < new_grids.size(); ++lev) {
    //     Print() << std::format("***** &new_grids[{:d}] = {}. size = {:d}\n", 
    //         lev, (void *) &(new_grids[lev]), new_grids[lev].size());
    // }
    ...
    for (int lev = lbase+1; lev <= new_finest; ++lev)
    {
        if (lev <= finest_level) // an old level
        {
            ...
            // DEBUG
            // ba_changed = true;
            // Print() << std::format("***** ba_changed = {} coarse_ba_changed = {}. {:s} RemakeLevel().\n", 
            //     ba_changed, coarse_ba_changed, (ba_changed || coarse_ba_changed) ? "Calling" : "Not calling");

            if (ba_changed || coarse_ba_changed) {
                const BoxArray& level_grids = ba_changed ? new_grids[lev] : grids[lev];
                const DistributionMapping& level_dmap = ba_changed ? DistributionMapping(level_grids) : dmap[lev];
            ...
            // DEBUG
            // Print() << std::format("***** &grids[{:d}] = {}. size = {:d}\n", 
            //     lev, (void *) &(grids[lev]), grids[lev].size());

        }
        else  // a new level
        {
            // DEBUG
            // Print() << std::format("***** lev={:d} is new. Calling MakeNewLevelFromCoarse({:d})\n", lev, lev);

            const BoxArray& level_grids = new_grids[lev];
            const DistributionMapping& level_dmap = DistributionMapping(level_grids);
            const auto old_num_setdm = num_setdm;
            SetBoxArray(lev, level_grids);
            if (old_num_setdm == num_setdm) {
                SetDistributionMap(lev, level_dmap);
            }
            // Call MakeNewLevelFromCoarse AFTER setting BoxArray and DistributionMapping
            // Doing it this way prevents segfaults that occur when MakeNewLevelFromCoarse was called first
            MakeNewLevelFromCoarse(lev, time, grids[lev], dmap[lev]);
    ...
    // DEBUG
    // Print() << std::format("AmrCore::regrid() complete.\nPrinting grid summary.\n");
