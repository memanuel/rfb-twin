...
{
    // MSE 2022-02-10: override set_eb_data::tiny with afrac_almost_zero (was 1.0e-16)
    // MSE 2022-02-10: override set_eb_data::almost_one with afrac_almost_one (was 1.0 - 1.0e-16)
    ...
    if (nxabs < afrac_almost_zero || nyabs > afrac_almost_one) {
        ...
        if (vfrac(i,j,0) > vfrac_almost_one) {
            vfrac(i,j,0) = 1.0;
            vcent(i,j,0,0) = 0.0;
            vcent(i,j,0,1) = 0.0;
        } else if (vfrac(i,j,0) < vfrac_almost_zero) {
            vfrac(i,j,0) = 0.0;
            vcent(i,j,0,0) = 0.0;
            vcent(i,j,0,1) = 0.0;
        } else {
...
void build_faces (Box const& bx, Array4<EBCellFlag> const& cell,
		          ...
		          Array4<Real> const& ifcx, Array4<Real> const& ifcy,
                  GpuArray<Real,AMREX_SPACEDIM> const& dx,
                  GpuArray<Real,AMREX_SPACEDIM> const& problo)
{
    // MSE 2022-02-10: override build_faces::small with afrac_almost_zero and afrac_almost-one (was 1.0e-14)
    ...
    AMREX_LAUNCH_HOST_DEVICE_LAMBDA ( ndbxg1, tbx,
    {
		...
		ifcx(i,j,0) = 0.0;
		    ...
		    ifcx(i,j,0) = 0.5*(1.0-apx(i,j,0)) - 0.5;
                } else {
                    apx(i,j,0) = 1.0 - (intery(i,j,0)-(problo[1]+j*dx[1]))*dyinv;
                    fcx(i,j,0) = 0.5 - 0.5*apx(i,j,0);
		    ifcx(i,j,0) = 0.5 - 0.5*(1.0-apx(i,j,0));
                }

                if (apx(i,j,0) > afrac_almost_one) {
                    apx(i,j,0) = 1.0;
                    fcx(i,j,0) = 0.0;
		            ifcx(i,j,0) = 0.0;
                    fx(i,j,0) = Type::regular;
                } else if (apx(i,j,0) < afrac_almost_zero) {
                    apx(i,j,0) = 0.0;
                    fcx(i,j,0) = 0.0;
		    ifcx(i,j,0) = 0.0;
		...
		ifcy(i,j,0) = 0.0;
		    ...
		    ifcy(i,j,0) = 0.5*(1.0-apy(i,j,0)) - 0.5;
                } else {
                    apy(i,j,0) = 1.0 - (interx(i,j,0)-(problo[0]+i*dx[0]))*dxinv;
                    fcy(i,j,0) = 0.5 - 0.5*apy(i,j,0);
		    ifcy(i,j,0) = 0.5 - 0.5*(1.0-apy(i,j,0));
                }

                if (apy(i,j,0) > afrac_almost_one) {
                    apy(i,j,0) = 1.0;
                    fcy(i,j,0) = 0.0;
		            ifcy(i,j,0) = 0.0;
                    fy(i,j,0) = Type::regular;
                } else if (apy(i,j,0) < afrac_almost_zero) {
                    apy(i,j,0) = 0.0;
                    fcy(i,j,0) = 0.0;
		            ifcy(i,j,0) = 0.0;
