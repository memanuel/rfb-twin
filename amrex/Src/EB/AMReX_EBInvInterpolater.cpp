...
namespace amrex {
  ...
  EBInvCellConservativeLinear  ebinv_lincc_interp;
  EBInvCellConservativeLinear  ebinv_cell_cons_interp(0);

  EBInvCellConservativeLinear::EBInvCellConservativeLinear (bool do_linear_limiting_)
    : CellConservativeLinear(do_linear_limiting_)
  {
  }

  EBInvCellConservativeLinear::~EBInvCellConservativeLinear ()
  {
  }

  //$$
  void
  EBInvCellConservativeLinear::interp (const FArrayBox& crse,
				       int              crse_comp,
				       FArrayBox&       fine,
				       int              fine_comp,
				       int              ncomp,
				       ...
				       RunOn            runon)
  {
	...
	if (ftype == FabType::multivalued || ctype == FabType::multivalued)
	  {
	    amrex::Abort("EBInvCellConservativeLinear::interp: multivalued not implemented");
	  }
	else //cut, or covered 
	  {
