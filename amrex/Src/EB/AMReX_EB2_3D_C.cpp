...
void cut_face_2d (Real& areafrac, Real& centx, Real& centy,
		          Real& icentx, Real& icenty,
                  Real& Sx2, Real& Sy2, Real& Sxy,
                  Real axm, Real axp, Real aym, Real ayp,
                  Real bcx, Real bcy) noexcept
{
    // MSE 2022-02-10: override build_faces::tiny with afrac_almost_zero (was 1.0e-16)
    // MSE 2022-02-10: override 1.0 - tiny with afrac_almost_zero (was 1.0 - 1.0e-16)
    ...
    if (nxabs < afrac_almost_zero || nyabs > afrac_almost_one) {
        areafrac = 0.5*(axm+axp);
        if (areafrac > afrac_almost_one) {
            areafrac = 1.0;
            centx = 0.0;
            centy = 0.0;
	    icentx = 0.0;
	    icenty = 0.0;
            Sx2 = Sy2 = 1./12.;
            Sxy = 0.0;
        } else if (areafrac < afrac_almost_zero) {
            areafrac = 0.0;
            centx = 0.0;
            centy = 0.0;
	    icentx = 0.0;
	    ...
	    icentx = 0.0;
            icenty = (-0.125*(ayp-aym) - ny*0.5*bcy*bcy)/(1.0-areafrac);
            Sx2 = (1./24.)*(axm+axp);
            Sy2 = (1./24.)*(ayp+aym) + ny*(1./3.)*(bcy*bcy*bcy);
            Sxy = 0.0;
        }
    } else if (nyabs < afrac_almost_zero || nxabs > afrac_almost_one) {
	    ...
	    icentx = 0.0;
            ...
            centy = 0.0;
	    icentx = (-0.125*(axp-axm) - nx*0.5*bcx*bcx)/(1.0-areafrac);
            icenty = 0.0;
	...
	icenty = -centy;
	    ...
	    icentx *= 1./(1.0-areafrac);
            icenty *= 1./(1.0-areafrac);
            icentx = amrex::min(amrex::max(icentx,-0.5),0.5);
            icenty = amrex::min(amrex::max(icenty,-0.5),0.5);
        }
    }
}
...
void build_faces (Box const& bx, Array4<EBCellFlag> const& cell,
		  ...
		  Array4<Real> const& ifcx, Array4<Real> const& ifcy, Array4<Real> const& ifcz,
                  Array4<Real> const& m2x, Array4<Real> const& m2y,
                  Array4<Real> const& m2z,
                  GpuArray<Real,AMREX_SPACEDIM> const& dx,
                  GpuArray<Real,AMREX_SPACEDIM> const& problo)
{
    // MSE 2022-02-10: override build_faces::small with afrac_small (was 1.0e-14)
    constexpr Real small = afrac_small;
    ...
    AMREX_HOST_DEVICE_FOR_3D ( xbx, i, j, k,
    {
		...
		ifcx(i,j,k,0) = 0.0;
                ...
                bcy = 0.5*bcy - 0.5;
                bcz = 0.5*bcz - 0.5;
                cut_face_2d(apx(i,j,k),fcx(i,j,k,0),fcx(i,j,k,1),ifcx(i,j,k,0),ifcx(i,j,k,1),
    ...
    AMREX_HOST_DEVICE_FOR_3D ( ybx, i, j, k,
    {
		...
		ifcy(i,j,k,1) = 0.0;
                ...
                bcx = 0.5*bcx - 0.5;
                bcz = 0.5*bcz - 0.5;
                cut_face_2d(apy(i,j,k),fcy(i,j,k,0),fcy(i,j,k,1),ifcy(i,j,k,0),ifcy(i,j,k,1),
    ...
    AMREX_HOST_DEVICE_FOR_3D ( zbx, i, j, k,
    {
		...
		ifcz(i,j,k,1) = 0.0;
                ...
                bcx = 0.5*bcx - 0.5;
                bcy = 0.5*bcy - 0.5;
                cut_face_2d(apz(i,j,k),fcz(i,j,k,0),fcz(i,j,k,1),ifcz(i,j,k,0),ifcz(i,j,k,1),
...
void build_cells (Box const& bx, Array4<EBCellFlag> const& cell,
		  ...
		  Array4<Real const> const& ifcx, Array4<Real const> const& ifcy, Array4<Real const> const& ifcz,
		  Array4<Real const> const& m2x,  Array4<Real const> const& m2y, Array4<Real const> const& m2z,
                  Array4<Real> const& vfrac, Array4<Real> const& vcent,
                  Array4<Real> const& barea, Array4<Real> const& bcent,
