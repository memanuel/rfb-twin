...
//    m_mgf.define(m_grids, m_dmap);
	...
	m_invfacecent[idim].define(amrex::convert(m_grids, IntVect::TheDimensionVector(idim)),
				   m_dmap, AMREX_SPACEDIM-1, ng);
    }
    ...
    rhs.fillInvFaceCent(amrex::GetArrOfPtrs(m_invfacecent), m_geom);
...
{
    // DEBUG
    // amrex::Print() << std::format("***** Level::coarsenFromFine - fill_boundary={:d}.\n", fill_boundary);
...
#pragma omp parallel reduction(max:mvmc_error)
        ...
        for (MFIter mfi(m_levelset,true); mfi.isValid(); ++mfi)
        {
            ...
            [=,&tile_error] (int i, int j, int k) noexcept
            {
                ...
                //         "***** Level::coarsenFromFine - ierror={:d}. (i,j,k) = ({:d}, {:d}, {:d}).\n", ierror, i, j, k);
                // }
            });
    ...
    auto& f_invfacecent = fineLevel.m_invfacecent;
    ...
    if (fill_boundary)
    {
	    ...
	    f_invfacecent[idim].FillBoundary(fine_period);
        }
...
#pragma omp parallel reduction(max:error)
	    ...
	    AMREX_D_TERM(auto const& cifcx = m_invfacecent[0].array(mfi);,
                         auto const& cifcy = m_invfacecent[1].array(mfi);,
                         auto const& cifcz = m_invfacecent[2].array(mfi););
	    ...
	    AMREX_D_TERM(auto const& fifcx = f_invfacecent[0].const_array(mfi);,
                         auto const& fifcy = f_invfacecent[1].const_array(mfi);,
                         auto const& fifcz = f_invfacecent[2].const_array(mfi););
...
void
Level::fillInvFaceCent (Array<MultiCutFab*,AMREX_SPACEDIM> const& a_invfacecent, const Geometry& geom) const
{
    if (isAllRegular()) {
        for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
            a_invfacecent[idim]->setVal(0.0);
        }        
        return;
    }
    ...
    for (int idim = 0; idim < AMREX_SPACEDIM; ++idim)
    {
        MultiCutFab& invfacecent = *a_invfacecent[idim];
        MultiFab tmp(invfacecent.boxArray(), invfacecent.DistributionMap(),
                     invfacecent.nComp(), invfacecent.nGrow());
        tmp.setVal(0.0);
        tmp.ParallelCopy(m_invfacecent[idim],0,0,invfacecent.nComp(),
                         0,invfacecent.nGrow(),geom.periodicity());
        copyMultiFabToMultiCutFab(invfacecent,tmp);
    }
}

void
Level::fillInvFaceCent (Array<MultiFab*,AMREX_SPACEDIM> const& a_invfacecent, const Geometry& geom) const
{
    for (int idim = 0; idim < AMREX_SPACEDIM; ++idim) {
        a_invfacecent[idim]->setVal(0.0);
    }
    if (!isAllRegular()) {
        for (int idim = 0; idim < AMREX_SPACEDIM; ++idim)
        {
            auto& invfacecent = *a_invfacecent[idim];
            a_invfacecent[idim]->ParallelCopy(m_invfacecent[idim],0,0,invfacecent.nComp(),
					      0,invfacecent.nGrow(),geom.periodicity());
        }
    }
}
