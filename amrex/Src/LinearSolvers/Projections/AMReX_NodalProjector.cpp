...
NodalProjector::setOptions ()
{
    ...
    // Print NodalProjector settings
    if (m_verbose > 2)
    {
        Print() << std::format("NodalProjector settings from namespace nodal_proj:\n");
        Print() << std::format("verbose = {}. bottom_verbose={}. maxiter={}. bottom_maxiter={}.\n", 
            m_verbose, bottom_verbose, maxiter, bottom_maxiter);
    ...
    else if (bottom_solver == "hypre")
    {
	    ...
	    m_mlmg->setHypreOptionsNamespace("nodal_proj.hypre");
    }
    #endif
}
