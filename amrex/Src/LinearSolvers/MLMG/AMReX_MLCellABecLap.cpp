...
std::unique_ptr<Hypre>
MLCellABecLap::makeHypre (Hypre::Interface hypre_interface, bool invert_eb) const
{
    const BoxArray& ba = m_grids[0].back();
    const DistributionMapping& dm = m_dmap[0].back();
    const Geometry& geom = m_geom[0].back();
    ...
    auto hypre_solver = amrex::makeHypre(ba, dm, geom, comm, hypre_interface, om, invert_eb);
