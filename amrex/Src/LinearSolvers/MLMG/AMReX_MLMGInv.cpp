#include <AMReX_MLMGInv.H>
#include <AMReX_MultiFabUtil.H>
#include <AMReX_VisMF.H>
#include <AMReX_BC_TYPES.H>
#include <AMReX_MLMGInv_K.H>
...
Real
MLMGInv::solve (const Vector<MultiFab*>& a_sol, const Vector<MultiFab const*>& a_rhs,
             Real a_tol_rel, Real a_tol_abs, const char* checkpoint_file)
{
    BL_PROFILE("MLMGInv::solve()");
    ...
    if (!is_nsolve) {
        ...
        if (verbose >= 1)
        {
            amrex::Print() << "MLMGInv: Initial rhs               = " << rhsnorm0 << "\n"
                           << "MLMGInv: Initial residual (resid0) = " << resnorm0 << "\n";
        }
    }
    ...
    if (!is_nsolve && resnorm0 <= res_target) {
        ...
        if (verbose >= 1) {
            amrex::Print() << "MLMGInv: No iterations needed\n";
        }
    } else {
        ...
        for (int iter = 0; iter < niters; ++iter)
        {
            ...
            if (verbose >= 2) {
                amrex::Print() << "MLMGInv: Iteration " << std::setw(3) << iter+1 << " Fine resid/"
            ...
            if (namrlevs == 1 && fine_converged) {
                ...
                if (verbose >= 2) {
                    amrex::Print() << "MLMGInv: Iteration " << std::setw(3) << iter+1
            ...
            if (converged) {
                if (verbose >= 1) {
                    amrex::Print() << "MLMGInv: Final Iter. " << iter+1
              ...
              {
                  if (verbose > 0) {
                      amrex::Print() << "MLMGInv: Failing to converge after " << iter+1 << " iterations."
                                     << " resid, resid/" << norm_name << " = "
                                     << composite_norminf << ", "
                                     << composite_norminf/max_norm << "\n";
                  }
		  amrex::Abort("MLMGInv failing so lets stop here");
              }
            }
        }
        ...
        if (!converged && do_fixed_number_of_iters == 0) {
            if (verbose > 0) {
                amrex::Print() << "MLMGInv: Failed to converge after " << max_iters << " iterations."
                               << " resid, resid/" << norm_name << " = "
                               << composite_norminf << ", "
                               << composite_norminf/max_norm << "\n";
            }
            amrex::Abort("MLMGInv failed");
        }
    ...
    if (verbose >= 1) {
        ...
        {
            amrex::AllPrint() << "MLMGInv: Timers: Solve = " << timer[solve_time]
...
void MLMGInv::oneIter (int iter)
{
    BL_PROFILE("MLMGInv::oneIter()");
...
void
MLMGInv::computeMLResidual (int amrlevmax)
{
    BL_PROFILE("MLMGInv::computeMLResidual()");
...
void
MLMGInv::computeResidual (int alev)
{
    BL_PROFILE("MLMGInv::computeResidual()");
...
void
MLMGInv::computeResWithCrseSolFineCor (int calev, int falev)
{
    BL_PROFILE("MLMGInv::computeResWithCrseSolFineCor()");
...
#ifdef AMREX_USE_EB
        amrex::EB_average_down(fine_res, crse_res, 0, ncomp, amrrr, invert_eb);
#else
...
void
MLMGInv::computeResWithCrseCorFineCor (int falev)
{
    BL_PROFILE("MLMGInv::computeResWithCrseCorFineCor()");
...
void
MLMGInv::miniCycle (int amrlev)
{
    BL_PROFILE("MLMGInv::miniCycle()");
    const int mglev = 0;
...
void
MLMGInv::mgVcycle (int amrlev, int mglev_top)
{
    BL_PROFILE("MLMGInv::mgVcycle()");

    const int mglev_bottom = linop.NMGLevels(amrlev) - 1;

    for (int mglev = mglev_top; mglev < mglev_bottom; ++mglev)
    {
        std::string blp_mgv_down_lev_str = make_str("MLMGInv::mgVcycle_down::", mglev);
    ...
    BL_PROFILE_VAR("MLMGInv::mgVcycle_bottom", blp_bottom);
    if (amrlev == 0)
    {
        if (verbose >= 4)
        {
    ...
    {
        std::string blp_mgv_up_lev_str = make_str("MLMGInv::mgVcycle_up::", mglev);
...
MLMGInv::mgFcycle ()
{
    ...
    {
#ifdef AMREX_USE_EB
        amrex::EB_average_down(res[amrlev][mglev-1], res[amrlev][mglev], 0, ncomp, linop.mg_coarsen_ratio_vec[mglev-1], invert_eb);
#else
        amrex::average_down(res[amrlev][mglev-1], res[amrlev][mglev], 0, ncomp, linop.mg_coarsen_ratio_vec[mglev-1]);
#endif
    }
...
void
MLMGInv::interpCorrection (int alev)
{
    BL_PROFILE("MLMGInv::interpCorrection_1");
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
        ...
        for (MFIter mfi(fine_cor, mfi_info); mfi.isValid(); ++mfi)
        {
            ...
            if (isEB)
            {
                ...
                if ( (flag.getType(amrex::grow(bx,1)) == FabType::regular && !invert_eb) || ( (flag.getType(amrex::grow(bx,1)) == FabType::covered) && invert_eb) ) {
                    call_lincc = true;
                } else {
                    ...
                    case 2:
                    {
                        ...
                        {
                            mlmg_eb_cc_interp_r<2>(tbx, ff, cc, flg, ncomp, invert_eb);
                        });
                        break;
                    }
                    case 4:
                    {
                        AMREX_LAUNCH_HOST_DEVICE_LAMBDA (bx, tbx,
                        {
                            mlmg_eb_cc_interp_r<4>(tbx, ff, cc, flg, ncomp, invert_eb);
                        });
                        break;
                    }
                    default:
...
void
MLMGInv::interpCorrection (int alev, int mglev)
{
    BL_PROFILE("MLMGInv::interpCorrection_2");
...
void
MLMGInv::addInterpCorrection (int alev, int mglev)
{
    BL_PROFILE("MLMGInv::addInterpCorrection()");
...
// out  : rescor
void
MLMGInv::computeResOfCorrection (int amrlev, int mglev)
{
    BL_PROFILE("MLMGInv:computeResOfCorrection()");
...
void
MLMGInv::NSolve (MLMGInv& a_solver, MultiFab& a_sol, MultiFab& a_rhs)
{
    BL_PROFILE("MLMGInv::NSolve()");
...
void
MLMGInv::actualBottomSolve ()
{
    BL_PROFILE("MLMGInv::actualBottomSolve()");
    ...
    else
    {
        ...
        else
        {
            ...
            // If the MLMGInv solve failed then set the correction to zero 
            if (ret != 0) {
                cor[amrlev][mglev]->setVal(0.0);
...
int
MLMGInv::bottomSolveWithCG (MultiFab& x, const MultiFab& b, MLCGSolver::Type type)
{
    ...
    if (ret != 0 && verbose > 1) {
        amrex::Print() << "MLMGInv: Bottom solve failed.\n";
    }
...
Real
MLMGInv::ResNormInf (int alev, bool local)
{
    BL_PROFILE("MLMGInv::ResNormInf()");
    const int ncomp = linop.getNComp();
    const int mglev = 0;
    Real norm = 0.0;
    ...
    if (linop.isCellCentered() && scratch[alev]) {
	    ...
	    if(!invert_eb) {
	      ...
	      MultiFab ivf(vfrac.boxArray(), vfrac.DistributionMap(), 1, 0);
	      MultiFab::Copy(ivf, vfrac, 0, 0, 1, 0);
	      ivf.negate();
	      ivf.plus(1.0, 0);
	      for (int n=0; n < ncomp; ++n) {
		MultiFab::Multiply(*pmf, ivf, 0, n, 1, 0);
	      }
	    }
        } else {
            amrex::Abort("MLMGInv::ResNormInf: not EB Factory");
        }
    }
#endif
    for (int n = 0; n < ncomp; n++)
...
Real
MLMGInv::MLResNormInf (int alevmax, bool local)
{
    BL_PROFILE("MLMGInv::MLResNormInf()");
    Real r = 0.0;
...
Real
MLMGInv::MLRhsNormInf (bool local)
{
    BL_PROFILE("MLMGInv::MLRhsNormInf()");
    const int ncomp = linop.getNComp();
    Real r = 0.0;
    for (int alev = 0; alev <= finest_amr_lev; ++alev)
    {
		...
		for (int n=0; n < ncomp; ++n) {
		  MultiFab::Multiply(*pmf, ivf, 0, n, 1, 0);
                ...
                amrex::Abort("MLMGInv::MLRhsNormInf: not EB Factory");
            }
        }
#endif
        for (int n=0; n<ncomp; ++n)
...
void
MLMGInv::buildFineMask ()
{
    BL_PROFILE("MLMGInv::buildFineMask()");
...
void
MLMGInv::prepareForSolve (const Vector<MultiFab*>& a_sol, const Vector<MultiFab const*>& a_rhs)
{
    BL_PROFILE("MLMGInv::prepareForSolve()");
    ...
    for (int alev = 0; alev < namrlevs; ++alev)
    {
        ...
        if (factory) {
            ...
            amrex::EB_set_covered(rhs[alev], 0, ncomp, val, invert_eb);
            amrex::EB_set_covered(*sol[alev], 0, ncomp, val, invert_eb);
        }
#endif
    }
    ...
    if (verbose >= 2) {
        amrex::Print() << "MLMGInv: # of AMR levels: " << namrlevs << "\n"
...
MLMGInv::prepareForNSolve ()
{
    ...
    ns_mlmg.reset(new MLMGInv(*ns_linop));
...
void
MLMGInv::getGradSolution (const Vector<Array<MultiFab*,AMREX_SPACEDIM> >& a_grad_sol,
                       Location a_loc)
{
    BL_PROFILE("MLMGInv::getGradSolution()");
    for (int alev = 0; alev <= finest_amr_lev; ++alev) {
...
void
MLMGInv::getFluxes (const Vector<Array<MultiFab*,AMREX_SPACEDIM> >& a_flux,
                 Location a_loc)
{
    if (!linop.isCellCentered()) {
...
void
MLMGInv::getFluxes (const Vector<MultiFab*> & a_flux, Location a_loc)
{
    AMREX_ASSERT(sol.size() == a_flux.size());
    getFluxes(a_flux, sol, a_loc);
}
...
void
MLMGInv::getFluxes (const Vector<MultiFab*> & a_flux, const Vector<MultiFab*>& a_sol, Location /*a_loc*/)
{
    ...
    if (linop.isCellCentered())
    {
        ...
        for (int alev = 0; alev < namrlevs; ++alev) {
#ifdef AMREX_USE_EB
            EB_average_face_to_cellcenter(*a_flux[alev], 0, amrex::GetArrOfConstPtrs(ffluxes[alev]),invert_eb);
#else
...
#ifdef AMREX_USE_EB
void
MLMGInv::getEBFluxes (const Vector<MultiFab*>& a_eb_flux)
{
    if (!linop.isCellCentered()) {
...
void
MLMGInv::getEBFluxes (const Vector<MultiFab*>& a_eb_flux, const Vector<MultiFab*>& a_sol)
{
    BL_PROFILE("MLMGInv::getEBFluxes()");
...
void
MLMGInv::compResidual (const Vector<MultiFab*>& a_res, const Vector<MultiFab*>& a_sol,
                    const Vector<MultiFab const*>& a_rhs)
{
    BL_PROFILE("MLMGInv::compResidual()");
    ...
    for (int alev = finest_amr_lev; alev >= 0; --alev) {
        ...
        if (alev < finest_amr_lev) {
            ...
            if (linop.isCellCentered()) {
#ifdef AMREX_USE_EB
                amrex::EB_average_down(*a_res[alev+1], *a_res[alev], 0, ncomp, amrrr[alev], invert_eb);
#else
...
void
MLMGInv::apply (const Vector<MultiFab*>& out, const Vector<MultiFab*>& a_in)
{
    BL_PROFILE("MLMGInv::apply()");
    ...
    for (int alev = finest_amr_lev; alev >= 0; --alev) {
        ...
        if (alev < finest_amr_lev) {
            ...
            if (linop.isCellCentered()) {
#ifdef AMREX_USE_EB
                amrex::EB_average_down(*out[alev+1], *out[alev], 0, out[alev]->nComp(), amrrr[alev], invert_eb);
#else
...
MLMGInv::averageDownAndSync ()
{
    ...
    if (linop.isCellCentered())
    {
        ...
        {
#ifdef AMREX_USE_EB
            amrex::EB_average_down(*sol[falev], *sol[falev-1], 0, ncomp, amrrr[falev-1], invert_eb);
#else
...
MLMGInv::computeVolInv ()
{
    ...
    { 
		...
		if(!invert_eb) {
		  ...
		  ivf.negate();
		  ivf.plus(1.0, 0);
		  volinv[amrlev][mglev] = ivf.sum(0,true);
		}
            }
            else
#endif
...
MLMGInv::makeSolvable ()
{
    ...
    if (linop.isCellCentered())
    {
	  ...
	  if(!invert_eb) {
	    ...
	    ivf.plus(1.0, 0);
	    for (int c = 0; c < ncomp; ++c) {
	      offset[c] = MultiFab::Dot(rhs[0], c, ivf, 0, 1, 0, true) * volinv[0][0];
	    }
	  }
        }
        else
        ...
        if (verbose >= 4) {
            for (int c = 0; c < ncomp; ++c) {
                amrex::Print() << "MLMGInv: Subtracting " << offset[c] 
    ...
    else
    {
        ...
        if (verbose >= 4) {
            amrex::Print() << "MLMGInv: Subtracting " << offset << " from rhs\n";
        }
        for (int alev = 0; alev < namrlevs; ++alev) {
...
void
MLMGInv::makeSolvable (int amrlev, int mglev, MultiFab& mf)
{
    const int ncomp = linop.getNComp();
    ...
    if (linop.isCellCentered())
    {
	  ...
	  if(!invert_eb) {
	    ...
	    ivf.plus(1.0, 0);
	    for (int c = 0; c < ncomp; ++c) {
	      offset[c] = MultiFab::Dot(mf, c, ivf, 0, 1, 0, true) * volinv[amrlev][mglev];
	    }
	  }
        }
        else
        ...
        if (mf.hasEBFabFactory()) {
            Vector<Real> val(ncomp, 0.0);
            amrex::EB_set_covered(mf, 0, ncomp, val, invert_eb);
        }
#endif
    }
    else
    {
        ...
        if (verbose >= 4) {
            amrex::Print() << "MLMGInv: Subtracting " << offset << " on level (" << amrlev << ", "
                           << mglev << ")\n";
        }
        mf.plus(-offset, 0, 1);
    }
...
Real
MLMGInv::getNodalSum (int amrlev, int mglev, MultiFab& mf) const
{
...
#if defined(AMREX_USE_HYPRE) && (AMREX_SPACEDIM > 1)
void
MLMGInv::bottomSolveWithHypre (MultiFab& x, const MultiFab& b)
{
    const int amrlev = 0;
    const int mglev  = linop.NMGLevels(amrlev) - 1;
    ...
    else
    {
        ...
        {
	    hypre_node_solver = linop.makeHypreNodeLap(bottom_verbose, hypre_options_namespace);
        }
...
void
MLMGInv::bottomSolveWithPETSc (MultiFab& x, const MultiFab& b)
{
...
void
MLMGInv::checkPoint (const Vector<MultiFab*>& a_sol, const Vector<MultiFab const*>& a_rhs,
