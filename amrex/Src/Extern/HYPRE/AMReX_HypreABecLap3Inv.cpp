...
HypreABecLap3Inv::HypreABecLap3Inv (const BoxArray& grids, const DistributionMapping& dmap,
				    const Geometry& geom_, MPI_Comm comm_,
				    const iMultiFab* overset_mask, const bool invert_eb)
    : Hypre(grids, dmap, geom_, comm_),
      m_overset_mask(overset_mask),
      m_invert_eb(invert_eb)
{}

HypreABecLap3Inv::~HypreABecLap3Inv ()
{}

void
HypreABecLap3Inv::solve (MultiFab& soln, const MultiFab& rhs, Real rel_tol, Real abs_tol,
			 int max_iter, const BndryData& bndry, int max_bndry_order)
{
    BL_PROFILE("HypreABecLap3Inv::solve()");
...
void
HypreABecLap3Inv::getSolution (MultiFab& a_soln)
{
...
void
HypreABecLap3Inv::prepareSolver ()
{
    ...
    auto fcent = (ebfactory) ? ( m_invert_eb ? ebfactory->getInvFaceCent() : ebfactory->getFaceCent() )
        : Array<const MultiCutFab*,AMREX_SPACEDIM>{AMREX_D_DECL(nullptr,nullptr,nullptr)};
...
#pragma omp parallel if (Gpu::notInLaunchRegion()) reduction(+:ncells_proc)
    ...
    AMREX_ALWAYS_ASSERT_WITH_MESSAGE(proc_end == proc_begin+ncells_proc,
                                     "HypreABecLap3Inv::prepareSolver: how did this happen?");
...
void
HypreABecLap3Inv::loadVectors (MultiFab& soln, const MultiFab& rhs)
{
    BL_PROFILE("HypreABecLap3Inv::loadVectors()");
    ...
    for (MFIter mfi(rhs_diag,TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        if (fabtyp == FabType::singlevalued) {
            ...
            if (osm) {
                ...
                {
		  rhs_diag_a(i,j,k) = (osm(i,j,k) == 0 || (flag(i,j,k).isCovered() && !m_invert_eb) || (flag(i,j,k).isRegular() && m_invert_eb) ) ?
                        Real(0.0) : rhs_a(i,j,k) * diaginv_a(i,j,k);
                });
            } else {
                AMREX_HOST_DEVICE_PARALLEL_FOR_3D_FUSIBLE(reg, i, j, k,
                {
                    rhs_diag_a(i,j,k) = ( (flag(i,j,k).isCovered() && !m_invert_eb) || (flag(i,j,k).isRegular() && m_invert_eb) ) ?
                        Real(0.0) : rhs_a(i,j,k) * diaginv_a(i,j,k);
                });
            }
