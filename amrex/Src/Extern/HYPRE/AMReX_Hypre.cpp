           ...
           const iMultiFab* overset_mask, const bool invert_eb)
{
  if (invert_eb==true) {
      if (overset_mask) {
	return std::unique_ptr<Hypre>(new HypreABecLap3Inv(grids, dmap, geom, comm_, overset_mask, invert_eb));
      }
      else {
	return std::unique_ptr<Hypre>(new HypreABecLap3Inv(grids, dmap, geom, comm_, nullptr, invert_eb));
      }
    }
    else {
      if (overset_mask) {
