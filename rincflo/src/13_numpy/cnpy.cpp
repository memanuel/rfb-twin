...
string cnpy::map_type(const type_info& t)
{
    // Bools are sui generis
    if(t == typeid(bool) ) 
        {return "b";}
    ...
    // Complex types
    if(t == typeid(complex<float>) ) 
        {return "c";}
    if(t == typeid(complex<double>) ) 
        {return "c";}
    if(t == typeid(complex<long double>) ) 
        {return "c";}

    // Exception - this will cause a Python program trying to read this field to crash later
    else return "?";
}
...
void cnpy::parse_npy_header(unsigned char* buffer,size_t& word_size, vector<size_t>& shape, bool& fortran_order) 
{
    //string magic_string(buffer,6);
    uint8_t major_version = *reinterpret_cast<uint8_t*>(buffer+6);
    uint8_t minor_version = *reinterpret_cast<uint8_t*>(buffer+7);
    uint16_t header_len = *reinterpret_cast<uint16_t*>(buffer+8);
    string header(reinterpret_cast<char*>(buffer+9),header_len);
    ...
    regex num_regex("[0-9][0-9]*");
    smatch sm;
    shape.clear();

    string str_shape = header.substr(loc1+1,loc2-loc1-1);
    while(regex_search(str_shape, sm, num_regex)) 
    {
        shape.push_back(stoi(sm[0].str()));
...
{  
    ...
    if (loc1 == string::npos)
        {throw runtime_error("parse_npy_header: failed to find header keyword: 'fortran_order'");}
    loc1 += 16;
    fortran_order = (header.substr(loc1,4) == "True" ? true : false);
    ...
    if (loc1 == string::npos || loc2 == string::npos)
        {throw runtime_error("parse_npy_header: failed to find header keyword: '(' or ')'");}
    ...
    if (loc1 == string::npos)
        {throw runtime_error("parse_npy_header: failed to find header keyword: 'descr'");}
    loc1 += 9;
    bool littleEndian = (header[loc1] == '<' || header[loc1] == '|' ? true : false);
    assert(littleEndian);
...
cnpy::NpyArray cnpy::npy_load(string fname) 
{
    ...
    if(!fp) 
        {throw runtime_error("npy_load: Unable to open file "+fname);}
...
cnpy::NpyArray load_the_npz_array(FILE* fp, uint32_t compr_bytes, uint32_t uncompr_bytes) 
{
    ...
    vector<unsigned char> buffer_compr(compr_bytes);
    vector<unsigned char> buffer_uncompr(uncompr_bytes);
...
cnpy::npz_t cnpy::npz_load(string fname) 
{
    ...
    if(!fp) 
        {throw runtime_error("npz_load: Error! Unable to open file "+fname+"!");}
...
cnpy::NpyArray cnpy::npz_load(string fname, string varname) 
{
    ...
    if(!fp) 
        {throw runtime_error("npz_load: Unable to open file "+fname);}
    ...
    throw runtime_error("npz_load: Variable name "+varname+" not found in "+fname);
}
