    ...
    Vector<Geometry> geom)
{
    ...
    #endif
    {
        for (MFIter mfi(vel, TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            bool is_solid = (typ == FabType::covered);
            bool is_liquid = (typ == FabType::regular);
            bool is_boundary = (typ == FabType::singlevalued) || (typ == FabType::multivalued);
            if (is_solid)
            {
                #if (AMREX_SPACEDIM == 3)
                auto func_ubx = 
                ...
                ParallelFor(ubx, vbx, wbx, func_ubx, func_vbx, func_wbx);
                #else
                auto func_ubx = 
                [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept { u(i,j,k) = 0.0; };
                auto func_vbx = 
                [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept { v(i,j,k) = 0.0; };
                ParallelFor(ubx, vbx, func_ubx, func_vbx);
                #endif
            }
            else if (is_boundary)
            {
                ...
                predict_vels_on_faces_eb(
                    lev, 
                    AMREX_D_DECL(ubx,vbx,wbx), 
                    AMREX_D_DECL(u,v,w),
                    vcc,flagarr,
                    AMREX_D_DECL(fcx,fcy,fcz),
                    ccc,h_bcrec,d_bcrec,geom);
            }
            else
            // liquid cells; or all cells when not using EB
            #endif
            {
    ...
    Vector<Geometry> geom)
{
    ...
    // This is only used because h_bcrec and d_bcrec hold the bc's for all three velocity components
    ...
    {
        ParallelFor(ubx, [vcc,domain_ilo,domain_ihi,u,d_bcrec]
    ...
    {
        ParallelFor(vbx, [vcc,domain_jlo,domain_jhi,v,d_bcrec]
        AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
    ...
    {
        ParallelFor(wbx, [vcc,domain_klo,domain_khi,w,d_bcrec]
        AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
