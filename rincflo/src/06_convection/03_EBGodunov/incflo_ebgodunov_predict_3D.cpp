...
void ebgodunov::predict_godunov_on_box (
    ...
    Real* p)
{
    ...
    ParallelFor(
        xebx, ncomp, func_xebx,
        yebx, ncomp, func_yebx,
        zebx, ncomp, func_zebx);
    ...
    ParallelFor(Box(divu), func_divu);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        ...
        EBGodunov_corner_couple_zy(
            l_zylo, l_zyhi, i, j, k, n, l_dt, dy, false,zlo(i,j,k,n), zhi(i,j,k,n), 
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        ...
        auto bc = pbc[n];
        Real l_yzlo, l_yzhi;
        EBGodunov_corner_couple_yz(
            l_yzlo, l_yzhi,i, j, k, n, l_dt, dz, false,  ylo(i,j,k,n), yhi(i,j,k,n), 
    ...
    ParallelFor(Box(zylo), Box(yzlo), func_zylo, func_yzlo);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        if (flag(i,j,k).isConnected(-1,0,0))
        {
            ...
            // Left side of interface
            {
                ...
                // Only add dt-based terms if we can construct all transverse terms using non-covered faces
                else if (apy(ic,j+1,k) > 0. && apy(ic,j,k) > 0. &&  apz(ic,j,k+1) > 0. && apz(ic,j,k) > 0.) 
                {
                    create_transverse_terms_for_xface(
                        ic, j, k, v_ad, w_ad, yzlo, zylo, apy, apz, fcy, fcz, trans_y, trans_z, dy, dz); 
                    stl += -0.5 * l_dt * (trans_y + trans_z) + 0.5 * l_dt * f(ic,j,k,n);
                } 
            }
            ...
            // Right side of interface
            {
                ...
                else if (apy(ic,j+1,k) > 0. && apy(ic,j,k) > 0. && apz(ic,j,k+1) > 0. && apz(ic,j,k) > 0.) 
                {
                    ...
                    sth += -0.5 * l_dt * (trans_y + trans_z)  +0.5 * l_dt * f(ic,j,k,n);
                }
            }
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        ...
        const auto bc = pbc[n];
        Real l_xzlo, l_xzhi;
        EBGodunov_corner_couple_xz(
            l_xzlo, l_xzhi, i, j, k, n, l_dt, dz, false, xlo(i,j,k,n), xhi(i,j,k,n),
            q, divu, apx, apy, apz, vfrac_arr, w_ad, zedge);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        ...
        const auto bc = pbc[n];
        Real l_zxlo, l_zxhi;
        EBGodunov_corner_couple_zx(
            l_zxlo, l_zxhi, i, j, k, n, l_dt, dx, false, zlo(i,j,k,n), zhi(i,j,k,n), 
    ...
    ParallelFor(Box(xzlo), Box(zxlo), func_xzlo, func_zxlo);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        if (flag(i,j,k).isConnected(0,-1,0))
        {
            ...
            // Left side of interface
            {
                ...
                else if (apx(i+1,jc,k  ) > 0. && apx(i,jc,k) > 0. &&  apz(i  ,jc,k+1) > 0. && apz(i,jc,k) > 0.) 
                {
                    create_transverse_terms_for_yface(
                        i, jc, k, u_ad, w_ad, xzlo, zxlo,  apx, apz, fcx, fcz, trans_x, trans_z, dx, dz); 
                    stl += -0.5 * l_dt * (trans_x + trans_z) + 0.5 * l_dt * f(i,jc,k,n);
                }
            }
            ...
            // Right side of interface
            {
                ...
                else if (apx(i+1,jc,k  ) > 0. && apx(i,jc,k) > 0. && apz(i  ,jc,k+1) > 0. && apz(i,jc,k) > 0.) 
                {
                    create_transverse_terms_for_yface(
                        i, jc, k, u_ad, w_ad, xzlo, zxlo, apx, apz, fcx, fcz, trans_x, trans_z, dx, dz); 
                    sth += -0.5 * l_dt * (trans_x + trans_z) + 0.5 * l_dt * f(i,jc,k,n);
                }
            }
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        ...
        const auto bc = pbc[n];
        Real l_xylo, l_xyhi;
        EBGodunov_corner_couple_xy(l_xylo, l_xyhi, i, j, k, n, l_dt, dy, false, xlo(i,j,k,n), xhi(i,j,k,n), 
            q, divu, apx, apy, apz, vfrac_arr, v_ad, yedge);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        ...
        const auto bc = pbc[n];
        Real l_yxlo, l_yxhi;
        EBGodunov_corner_couple_yx(l_yxlo, l_yxhi, i, j, k, n, l_dt, dx, false, ylo(i,j,k,n), yhi(i,j,k,n),
            q, divu, apx, apy, apz, vfrac_arr, u_ad, xedge);
    ...
    ParallelFor(Box(xylo), Box(yxlo), func_xylo, func_yxlo);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        if (flag(i,j,k).isConnected(0,0,-1))
        {
            ...
            if (flag(i,j,kc).isRegular())
            {
                ...
                else if (apx(i+1,j  ,kc) > 0. && apx(i,j,kc) > 0. &&  apy(i  ,j+1,kc) > 0. && apy(i,j,kc) > 0.) 
                {
                    create_transverse_terms_for_zface(
                        i, j, kc, u_ad, v_ad, xylo, yxlo,  apx, apy, fcx, fcy, trans_x, trans_y, dx, dy); 
                    stl += -0.5 * l_dt * (trans_x + trans_y)  +0.5 * l_dt * f(i,j,kc,n);
                }
            }
            ...
            // Right side of interface
            {
                ...
                else if (apx(i+1,j  ,kc) > 0. && apx(i,j,kc) > 0. &&  apy(i  ,j+1,kc) > 0. && apy(i,j,kc) > 0.) 
                {
                    create_transverse_terms_for_zface(
                        i, j, kc, u_ad, v_ad, xylo, yxlo,  apx, apy, fcx, fcy, trans_x, trans_y, dx, dy); 
                    sth += -0.5 * l_dt * (trans_x + trans_y) +0.5 * l_dt * f(i,j,kc,n);
                }
            }
            Real gphi_z;
