...
// *********************************************************************************************************************
void ebgodunov::compute_godunov_fluxes (
    ...
    bool is_velocity )
{
    ...
    {
       if (!iconserv[n]) 
        {Abort("Trying to update in non-conservative in ebgodunov");}
    }
    ...
    for (int n = 0; n < ncomp; n++)
       if (!iconserv[n]) Abort("Trying to update in non-conservative in ebgodunov");
    ...
    ParallelFor(Box(Imx), ncomp, func_imx);
    ...
    ParallelFor(Box(Imy), ncomp, func_imy);
    ...
    ParallelFor(Box(Imz), ncomp, func_imz);

    ebgodunov::plm_fpu_x (
        xebx, ncomp, Imx, Ipx, q, umac,  flag_arr, vfrac_arr,
        AMREX_D_DECL(fcx,fcy,fcz),ccent_arr, geom, l_dt, h_bcrec, pbc, is_velocity);
    ebgodunov::plm_fpu_y (
        yebx, ncomp, Imy, Ipy, q, vmac, flag_arr, vfrac_arr,
        AMREX_D_DECL(fcx,fcy,fcz),ccent_arr, geom, l_dt, h_bcrec, pbc, is_velocity);
    ebgodunov::plm_fpu_z (
        zebx, ncomp, Imz, Ipz, q, wmac, flag_arr, vfrac_arr,
        AMREX_D_DECL(fcx,fcy,fcz),ccent_arr, geom, l_dt, h_bcrec, pbc, is_velocity);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        const auto bc = pbc[n];
        Real l_zylo, l_zyhi;
        EBGodunov_corner_couple_zy( l_zylo, l_zyhi, i, j, k, n, l_dt, dy, true,  zlo(i,j,k,n), zhi(i,j,k,n), 
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        const auto bc = pbc[n];
        Real l_yzlo, l_yzhi;
        EBGodunov_corner_couple_yz( l_yzlo, l_yzhi, i, j, k, n, l_dt, dz, true, ylo(i,j,k,n), yhi(i,j,k,n),
    ...
    ParallelFor(
    Box(zylo), ncomp, func_zylo,
    Box(yzlo), ncomp, func_yzlo);
    ...
    ParallelFor(xbx, ncomp, func_xbx);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        ...
        EBGodunov_corner_couple_xz(
            l_xzlo, l_xzhi, i, j, k, n, l_dt, dz, true, 
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        const auto bc = pbc[n];
        Real l_zxlo, l_zxhi;
        EBGodunov_corner_couple_zx( l_zxlo, l_zxhi, i, j, k, n, l_dt, dx, true, zlo(i,j,k,n), zhi(i,j,k,n),
    ...
    ParallelFor(
    Box(xzlo), ncomp, func_xzlo,
    Box(zxlo), ncomp, func_zxlo);
    //
    ...
    ParallelFor(ybx, ncomp, func_ybx);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        const auto bc = pbc[n];
        Real l_xylo, l_xyhi;
        EBGodunov_corner_couple_xy(
            l_xylo, l_xyhi, i, j, k, n, l_dt, dy, true, 
            xlo(i,j,k,n), xhi(i,j,k,n),
            q, divu, apx, apy, apz, vfrac_arr, vmac, yedge);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
    {
        const auto bc = pbc[n];
        Real l_yxlo, l_yxhi;
        EBGodunov_corner_couple_yx(
            l_yxlo, l_yxhi, i, j, k, n, l_dt, dx, true, 
    ...
    ParallelFor( 
    Box(xylo), ncomp, func_xylo,
    Box(yxlo), ncomp, func_yxlo);
    ...
    ParallelFor(zbx, ncomp, func_zbx);
}
