...
void ebgodunov::plm_fpu_x (
    Box const& xebox, int ncomp,
    ...
    bool is_velocity)
{
    ...
    if ( (has_extdir_or_ho_lo_x and domain_ilo >= xebox.smallEnd(0)-1) ||
         (has_extdir_or_ho_hi_x and domain_ihi <= xebox.bigEnd(0)    ) ||
         #if (AMREX_SPACEDIM == 3)
         (has_extdir_or_ho_lo_z and domain_klo >= xebox.smallEnd(2)-1) ||
         (has_extdir_or_ho_hi_z and domain_khi <= xebox.bigEnd(2)    ) ||
         #endif
         (has_extdir_or_ho_lo_y and domain_jlo >= xebox.smallEnd(1)-1) ||
         (has_extdir_or_ho_hi_y and domain_jhi <= xebox.bigEnd(1)    )  )
    {
        ParallelFor(xebox, ncomp, 
        [   q,umac,AMREX_D_DECL(domain_ilo,domain_jlo,domain_klo), AMREX_D_DECL(domain_ihi,domain_jhi,domain_khi),
            Imx,Ipx,dtdx,pbc,flag,vfrac,ccc,AMREX_D_DECL(fcx,fcy,fcz), is_velocity]
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            ...
            {
                const auto& bc = pbc[n];
                bool extdir_or_ho_ilo = (bc.lo(0) == BCType::ext_dir) || (bc.lo(0) == BCType::hoextrap);
                bool extdir_or_ho_ihi = (bc.hi(0) == BCType::ext_dir) || (bc.hi(0) == BCType::hoextrap);
                bool extdir_or_ho_jlo = (bc.lo(1) == BCType::ext_dir) || (bc.lo(1) == BCType::hoextrap);
                bool extdir_or_ho_jhi = (bc.hi(1) == BCType::ext_dir) || (bc.hi(1) == BCType::hoextrap);
                #if (AMREX_SPACEDIM == 3)
                bool extdir_or_ho_klo = (bc.lo(2) == BCType::ext_dir) || (bc.lo(2) == BCType::hoextrap);
                bool extdir_or_ho_khi = (bc.hi(2) == BCType::ext_dir) || (bc.hi(2) == BCType::hoextrap);
                #endif
                ...
                else 
                {
                    ...
                    const auto& slopes_eb_hi = amrex_lim_slopes_extdir_eb(
                        i,j,k,n,q,ccc,
                ...
                else 
                {
                    ...
                    #endif
                    AMREX_D_TERM(
                    Real delta_x = 0.5 - ccc(i-1,j,k,0);,
                    Real delta_y = yf  - ccc(i-1,j,k,1);,
                    Real delta_z = zf  - ccc(i-1,j,k,2);)
    
                    const auto& slopes_eb_lo = amrex_lim_slopes_extdir_eb(
                        i-1,j,k,n,q,ccc,
                        AMREX_D_DECL(fcx,fcy,fcz), 
                        flag,
                        AMREX_D_DECL(extdir_or_ho_ilo, extdir_or_ho_jlo, extdir_or_ho_klo),
    ...
    else // The cases below are not near any domain boundary
    {
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            ...
            if (flag(i,j,k).isConnected(-1,0,0))
            {
                ...
                {
                    int order = 4;
                    qpls = q(i,j,k,n) + 0.5 * (-1.0 - umac(i,j,k,0) * dtdx) * amrex_calc_xslope(i,j,k,n,order,q);
                ...
                else 
                {
                    ...
                    Real delta_y = yf  - ccc(i,j,k,1);,
                    Real delta_z = zf  - ccc(i,j,k,2);)    
                    const auto& slopes_eb_hi = amrex_lim_slopes_eb(
                        i,j,k,n,q,ccc,
                ...
                {
                    int order = 4;
                    qmns = q(i-1,j,k,n) + 0.5 * ( 1.0 - umac(i,j,k) * dtdx) * amrex_calc_xslope(i-1,j,k,n,order,q);
                } 
                // We have enough cells to do 2nd order slopes with all values at cell centers
                else if (vfrac(i-1,j,k) == 1. and vfrac(i-2,j,k) == 1. and vfrac(i  ,j,k) == 1.) 
                {
                ...
                else 
                {
                    ...
                    Real delta_y = yf  - ccc(i-1,j,k,1);,
                    Real delta_z = zf  - ccc(i-1,j,k,2);)
                    const auto& slopes_eb_lo = amrex_lim_slopes_eb(
                        i-1,j,k,n,q,ccc, AMREX_D_DECL(fcx,fcy,fcz), flag);
                    #if (AMREX_SPACEDIM == 3)
                    qmns = q(i-1,j,k,n) + delta_x * slopes_eb_lo[0]
                                        + delta_y * slopes_eb_lo[1]
                                        + delta_z * slopes_eb_lo[2];
...
// *********************************************************************************************************************
// This version is called after the MAC projection
void ebgodunov::plm_fpu_y (
    Box const& yebox, int ncomp,
    ...
    BCRec const* pbc, bool is_velocity)
{
    ...
    if ( (has_extdir_or_ho_lo_x and domain_ilo >= yebox.smallEnd(0)-1) ||
         (has_extdir_or_ho_hi_x and domain_ihi <= yebox.bigEnd(0)    ) ||
    #if (AMREX_SPACEDIM == 3)
         (has_extdir_or_ho_lo_z and domain_klo >= yebox.smallEnd(2)-1) ||
         (has_extdir_or_ho_hi_z and domain_khi <= yebox.bigEnd(2)    ) ||
    #endif
         (has_extdir_or_ho_lo_y and domain_jlo >= yebox.smallEnd(1)-1) ||
         (has_extdir_or_ho_hi_y and domain_jhi <= yebox.bigEnd(1)    )  )
    {
        ParallelFor(yebox, ncomp, 
        [   q,vmac,AMREX_D_DECL(domain_ilo,domain_jlo,domain_klo), AMREX_D_DECL(domain_ihi,domain_jhi,domain_khi),
            Imy,Ipy,dtdy,pbc,flag,vfrac,ccc,AMREX_D_DECL(fcx,fcy,fcz), is_velocity]
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            Real qpls(0.);
            Real qmns(0.);
            ...
            if (flag(i,j,k).isConnected(0,-1,0))
            {
                ...
                else 
                {
                    ...
                    #endif
                    AMREX_D_TERM(
                    Real delta_y = 0.5 - ccc(i,j-1,k,1);,
    ...
    // The cases below are not near any domain boundary
    {
        ParallelFor(yebox, ncomp, [q,vmac,AMREX_D_DECL(domain_ilo,domain_jlo,domain_klo),
                                                AMREX_D_DECL(domain_ihi,domain_jhi,domain_khi),
        ...
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            ...
            if (flag(i,j,k).isConnected(0,-1,0))
            {
                ...
                {
                    int order = 2;
                    qpls = q(i,j,k,n) + 0.5 * (-1.0 - vmac(i,j,k) * dtdy) * amrex_calc_yslope(i,j,k,n,order,q);
                } 
                // We need to use LS slopes
                else 
                {
...
#if (AMREX_SPACEDIM == 3)
// This version is called after the MAC projection
void ebgodunov::plm_fpu_z (
    Box const& zebox, int ncomp,
    ...
    BCRec const* pbc, bool is_velocity)
{
    ...
    if ( (has_extdir_or_ho_lo_x and domain_ilo >= zebox.smallEnd(0)-1) ||
         (has_extdir_or_ho_hi_x and domain_ihi <= zebox.bigEnd(0)    ) ||
         (has_extdir_or_ho_lo_z and domain_klo >= zebox.smallEnd(2)-1) ||
         (has_extdir_or_ho_hi_z and domain_khi <= zebox.bigEnd(2)    ) ||
         (has_extdir_or_ho_lo_y and domain_jlo >= zebox.smallEnd(1)-1) ||
         (has_extdir_or_ho_hi_y and domain_jhi <= zebox.bigEnd(1)    )  )
    {
        ParallelFor(zebox, ncomp, 
        [   q,wmac,AMREX_D_DECL(domain_ilo,domain_jlo,domain_klo), AMREX_D_DECL(domain_ihi,domain_jhi,domain_khi),
            Imz,Ipz,dtdz,pbc,flag,vfrac,ccc,AMREX_D_DECL(fcx,fcy,fcz), is_velocity]
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            Real qpls(0.);
            Real qmns(0.);
            ...
            if (flag(i,j,k).isConnected(0,0,-1))
            {
                ...
                else 
                {
                    ...
                    AMREX_D_TERM(
                    Real delta_z = 0.5 - ccc(i,j,k-1,2);,
    ...
    // The cases below are not near any domain boundary
    {
        ParallelFor(zebox, ncomp, 
        [   q,wmac,AMREX_D_DECL(domain_ilo,domain_jlo,domain_klo), AMREX_D_DECL(domain_ihi,domain_jhi,domain_khi),
            Imz,Ipz,dt,dtdz,pbc,flag,vfrac,ccc,AMREX_D_DECL(fcx,fcy,fcz), is_velocity]
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            Real qpls(0.);
            Real qmns(0.);
            ...
            if (flag(i,j,k).isConnected(0,0,-1))
            {
                ...
                {
                    int order = 4;
                    qpls = q(i,j,k,n) + 0.5 * (-1.0 - wmac(i,j,k) * dtdz) * amrex_calc_zslope(i,j,k,n,order,q);
                } 
                // We have enough cells to do 2nd order slopes with all values at cell centers
                else if (vfrac(i,j,k) == 1. and vfrac(i,j,k-1) == 1. and vfrac(i,j,k+1) == 1.) {
