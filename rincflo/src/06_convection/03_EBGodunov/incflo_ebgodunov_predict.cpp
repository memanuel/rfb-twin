...
// *********************************************************************************************************************
void ebgodunov::predict_godunov (
    Real /*time*/,
    AMREX_D_DECL(
    MultiFab& u_mac, 
    MultiFab& v_mac,
    ...
    bool use_mac_phi_in_godunov)
{
    ...
    #endif
    {
        ...
        for (MFIter mfi(vel,TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            {
                godunov::predict_plm_x (xebx_g2, Imx, Ipx, a_vel, a_vel, geom, l_dt, h_bcrec, d_bcrec);
                godunov::predict_plm_y (yebx_g2, Imy, Ipy, a_vel, a_vel, geom, l_dt, h_bcrec, d_bcrec);
                #if (AMREX_SPACEDIM == 3)
                godunov::predict_plm_z (zebx_g2, Imz, Ipz, a_vel, a_vel, geom, l_dt, h_bcrec, d_bcrec);
                #endif

                bool use_forces_in_trans = false;
                godunov::make_trans_velocities(
                    AMREX_D_DECL(Box(u_ad), Box(v_ad), Box(w_ad)),
                    AMREX_D_DECL(u_ad, v_ad, w_ad),
                    AMREX_D_DECL(Imx, Imy, Imz), 
                    AMREX_D_DECL(Ipx, Ipy, Ipz), 
                    a_vel, a_f, domain, l_dt, d_bcrec, use_forces_in_trans);

                godunov::predict_godunov_on_box(
                    bx, ncomp, 
                    ...
                    AMREX_D_DECL(Imx, Imy, Imz), 
                    AMREX_D_DECL(Ipx, Ipy, Ipz), 
                    a_f, domain, dx, l_dt, d_bcrec, use_forces_in_trans,
            ...
            else 
            {
                ...
                ebgodunov::predict_plm_x(
                    xebx_g2, Imx, Ipx, a_vel, a_vel,
                    flagarr, vfrac_arr,
                    AMREX_D_DECL(fcx,fcy,fcz),
                    ccent_arr, geom, l_dt, h_bcrec, d_bcrec);
                ebgodunov::predict_plm_y(
                    yebx_g2, Imy, Ipy, a_vel, a_vel,
                    flagarr, vfrac_arr,
                    AMREX_D_DECL(fcx,fcy,fcz),
                    ccent_arr, geom, l_dt, h_bcrec, d_bcrec);
                #if (AMREX_SPACEDIM == 3)
                ebgodunov::predict_plm_z(
                    zebx_g2, Imz, Ipz, a_vel, a_vel,
                    flagarr, vfrac_arr,
                    AMREX_D_DECL(fcx,fcy,fcz),
                    ccent_arr, geom, l_dt, h_bcrec, d_bcrec);
                #endif

                ebgodunov::make_trans_velocities(
                ...
                ebgodunov::predict_godunov_on_box(
                    bx, ncomp, 
                    AMREX_D_DECL(xbx,ybx,zbx),
                    AMREX_D_DECL(xebx_g2,yebx_g2,zebx_g2),
                    AMREX_D_DECL(a_umac, a_vmac, a_wmac),
                    ...
                    AMREX_D_DECL(Imx, Imy, Imz), 
                    AMREX_D_DECL(Ipx, Ipy, Ipz), 
                    a_f, domain, dx, l_dt, d_bcrec, flagarr,
