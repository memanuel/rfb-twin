...
// *********************************************************************************************************************
void redistribution::redistribute_eb (
    ...
    Geometry& lev_geom, Real dt, std::string redistribution_type)
{
    ...
    if (redistribution_type == "FluxRedist")
    {
        ...
        ParallelFor(Box(scratch), func_scratch);
    ...
    else if (redistribution_type == "MergeRedist") 
    {
        ...
        ParallelFor(Box(dUdt_in), ncomp, func_in);
    ...
    else if (redistribution_type == "StateRedist") 
    {
        ...
        if (!domain_per_grown.contains(bxg1))
        {
            ...
            ParallelFor(bxg1,ncomp, func_bxg1);
        }
        auto func_scratch = 
        [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
        ...
        ParallelFor(Box(scratch), ncomp,  func_scratch);
...
// *********************************************************************************************************************
void redistribution::RedistributeInitialData (
    Box const& bx, int ncomp,
    ...
    Geometry& lev_geom, std::string redistribution_type)
{
    ...
    {
        make_itracker(bx, AMREX_D_DECL(apx, apy, apz), vfrac, itr, lev_geom, "State");
        state_redistribute(bx, ncomp, U_out, U_in, flag, vfrac, AMREX_D_DECL(fcx, fcy, fcz), ccc, itr, lev_geom);
    } 
    else 
