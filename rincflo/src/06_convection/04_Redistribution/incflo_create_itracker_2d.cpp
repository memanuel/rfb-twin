...
//  if (debug_verbose > 0)
//      Print() << " IN MAKE_ITRACKER DOING BOX " << bx << std::endl;
    ...
    ParallelFor(Box(itracker), func_it);
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
       if (vfrac(i,j,k) > 0.0 && vfrac(i,j,k) < 0.5)
       {
           ...
           if ( (!xdir_mns_ok && (itracker(i,j,k,1) == 4)) ||  (!xdir_pls_ok && (itracker(i,j,k,1) == 5)) )
           {
               itracker(i,j,k,1) = (ny > 0) ? 7 : 2;
           }
           if ( (!ydir_mns_ok && (itracker(i,j,k,1) == 2)) || (!ydir_pls_ok && (itracker(i,j,k,1) == 7)) )
           {
           ...
           if (vfrac(i+ioff,j+joff,k) == 0.)
               Abort(" Trying to merge with covered cell");
           ...
           if (sum_vol < 0.5)
           {
             Abort("Couldnt merge with enough cells to raise volume greater than 0.5");
           }
       }
    };
