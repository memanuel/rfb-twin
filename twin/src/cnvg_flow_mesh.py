...
# Local imports
from cnvg_utils import SimOutput, geom_name_tbl, subdir_tbl, plot_dir, fit_error_mesh
...
# *************************************************************************************************
def vel_diff_l2(s_ref: SimOutput, s: SimOutput) -> float:
    """
...
# *************************************************************************************************
def vel_error(G: int, nz_ref: int, nzs_comp: list[int]) \
    ...
    for nz_i in nzs_comp:
        ...
        for level_i in range(level_max+1):
            ...
            err_i: float = vel_diff_l2(s_ref=s_ref, s=s)
...
# *************************************************************************************************
def plot_vel_error(nz: NumpyInt16Array, level: NumpyInt16Array, err: NumpyFloatArray, 
                   nz_ref: int, G: int, fig_style: bool, verbose: bool) -> None:
    """
    ...
    if verbose:
        print(f'Errors for flow convergence with mesh refinement and geometry {G:d}:')
    ...
    hh_c_L4 = np.logspace(log2h_min+2.75, log2h_max, 17, base=2.0)
    ...
    # Corresponding values of refinement ratio r = h_ref / h_c; measure this on the coarse grid
    rr_L0 = h_ref / hh_c_L0
    rr_L1 = h_ref / hh_c_L1
    rr_L2 = h_ref / hh_c_L2
    rr_L3 = h_ref / hh_c_L3
    ...
    err_fit_L0 = \
        C_c * np.power(hh_c_L0, p) * (1.0 - alpha * np.power(rr_L0, p)) + \
        C_f * np.power(hh_f_L0, p) * (1.0 - alpha * np.power(rr_L0, p))
    err_fit_L1 = \
        C_c * np.power(hh_c_L1, p) * (1.0 - alpha * np.power(rr_L1, p)) + \
        C_f * np.power(hh_f_L1, p) * (1.0 - alpha * np.power(rr_L1, p))
    err_fit_L2 = \
        C_c * np.power(hh_c_L2, p) * (1.0 - alpha * np.power(rr_L2, p)) + \
        C_f * np.power(hh_f_L2, p) * (1.0 - alpha * np.power(rr_L2, p))
    err_fit_L3 = \
        C_c * np.power(hh_c_L3, p) * (1.0 - alpha * np.power(rr_L3, p)) + \
        C_f * np.power(hh_f_L3, p) * (1.0 - alpha * np.power(rr_L3, p))
    err_fit_L4 = \
        C_c * np.power(hh_c_L4, p) * (1.0 - alpha * np.power(rr_L4, p)) + \
        C_f * np.power(hh_f_L4, p) * (1.0 - alpha * np.power(rr_L4, p))
    ...
    line_L0 = ax.plot(h[mask_L0], err[mask_L0], marker='o', markersize=markersize, linestyle='None', label='L0')
    line_L1 = ax.plot(h[mask_L1], err[mask_L1], marker='o', markersize=markersize, linestyle='None', label='L1')
    line_L2 = ax.plot(h[mask_L2], err[mask_L2], marker='o', markersize=markersize, linestyle='None', label='L2')
    line_L3 = ax.plot(h[mask_L3], err[mask_L3], marker='o', markersize=markersize, linestyle='None', label='L3')
    line_L4 = ax.plot(h[mask_L4], err[mask_L4], marker='o', markersize=markersize, linestyle='None', label='L4')

    # Plot the fit for each level of refinement
    linewidth = 2.0 if fig_style else 1.0
    ax.plot(hh_c_L0, err_fit_L0, linestyle='--', linewidth=linewidth, label=None, color=line_L0[0].get_color())
    ax.plot(hh_c_L1, err_fit_L1, linestyle='--', linewidth=linewidth, label=None, color=line_L1[0].get_color())
    ax.plot(hh_c_L2, err_fit_L2, linestyle='--', linewidth=linewidth, label=None, color=line_L2[0].get_color())
    ax.plot(hh_c_L3, err_fit_L3, linestyle='--', linewidth=linewidth, label=None, color=line_L3[0].get_color())
    ax.plot(hh_c_L4, err_fit_L4, linestyle='--', linewidth=linewidth, label=None, color=line_L4[0].get_color())
    ...
    # Set plot title
    if not fig_style:
        ax.set_title(f'Flow Convergence with Refinement - {geom_name:s}', fontsize=20)
    else:
        ax.set_title(None)
    # Set axis labels
    fontsize: int = 20 if fig_style else 14
    ax.set_xlabel('Relative Step Size $h$ on Coarse Grid (dimensionless)', fontsize=fontsize)
    ...
    fontsize = 13 if fig_style else 10
    ...
    # Save the figure
    fname: str = (plot_dir / f'{plot_num:02d}_flow_cnvg_mesh.png').as_posix()
...
def main():
    ...
    for G in Gs:
        ...
        err: NumpyFloatArray  
        nz, level, err = vel_error(G=G, nz_ref=nz_ref, nzs_comp=nzs_comp)
        # Plot the velocity error
        plot_vel_error(nz=nz, err=err, nz_ref=nz_ref, G=G, level=level, fig_style=fig_style, verbose=verbose)
